...
use self::state::TokenizerState;

use self::token::{
  LitKind, NumberBase,
  Token, TokenKind, TokenPrinter, TokenQueue, TokenSink,
};

use self::util::ascii::*;
use self::util::smallcharset::{small_char_set, SmallCharSet};
...
pub struct TokenizerOpts {
  ...
  pub initial_state: Option<state::TokenizerState>,
  pub profile: bool,
  pub safe_mod: bool,
}
...
impl<Sink: TokenSink> Tokenizer<Sink> {
  pub fn new(sink: Sink, opts: TokenizerOpts) -> Tokenizer<Sink> {
    let state = *opts.initial_state.as_ref().unwrap_or(&state::Start);
  ...
  fn add(&mut self, kind: TokenKind, span: Span) {
    self.token_queue.push_back(Token::new(kind.clone(), span.clone()));
